"use strict";
// ---------------------------------------------------------------------------
// KDP print specification — the ONLY place KDP-related measurements live.
// If Amazon changes its requirements, update this module and nothing else.
// All measurements are in inches unless stated otherwise.
// ---------------------------------------------------------------------------
Object.defineProperty(exports, "__esModule", { value: true });
exports.PDF_POINTS_PER_INCH = exports.KDP_MAX_PAGE_COUNT_BW = exports.KDP_MIN_PAGE_COUNT = exports.MAX_GENERATION_RETRIES = exports.MAX_CONCURRENT_GENERATIONS = exports.DEFAULT_PAPER_TYPE = exports.SPINE_RATES = exports.MIN_SPINE_TEXT_WIDTH_IN = exports.BARCODE_AREA = exports.COVER_SAFE_MARGIN_IN = exports.COVER_BLEED_IN = exports.DEFAULT_INTERIOR_MARGIN_ID = exports.INTERIOR_MARGINS = exports.INTERIOR_IMAGE = exports.DEFAULT_TRIM_SIZE_ID = exports.TRIM_SIZES = exports.PRINT_DPI = void 0;
exports.calculateSpineWidthIn = calculateSpineWidthIn;
exports.calculateCoverDimensions = calculateCoverDimensions;
exports.PRINT_DPI = 300;
/** Supported trim sizes. Add new entries here to support more sizes. */
exports.TRIM_SIZES = {
    "8.5x11": {
        id: "8.5x11",
        label: '8.5 × 11 in — US Letter',
        widthIn: 8.5,
        heightIn: 11,
    },
};
exports.DEFAULT_TRIM_SIZE_ID = "8.5x11";
/** Final normalised interior image size: 8.5 × 11 in at 300 DPI. */
exports.INTERIOR_IMAGE = {
    widthPx: 2550,
    heightPx: 3300,
    dpi: exports.PRINT_DPI,
};
/**
 * Interior margin presets. The colouring-book preset uses a no-bleed
 * interior because illustrations stay safely inside a white margin.
 * KDP minimums (no bleed): outside/top/bottom ≥ 0.25", gutter depends on
 * page count — 0.375" covers up to 150 pages; kept generous here.
 */
exports.INTERIOR_MARGINS = {
    colouringBookNoBleed: {
        insideIn: 0.5,
        outsideIn: 0.375,
        topIn: 0.375,
        bottomIn: 0.375,
        bleedIn: 0,
    },
};
exports.DEFAULT_INTERIOR_MARGIN_ID = "colouringBookNoBleed";
/** KDP cover bleed (applied on all four outer edges of the wraparound). */
exports.COVER_BLEED_IN = 0.125;
/** Keep text/important cover elements this far inside the trim edge. */
exports.COVER_SAFE_MARGIN_IN = 0.25;
/** Amazon places its barcode on the back cover; keep this area clear. */
exports.BARCODE_AREA = {
    widthIn: 2,
    heightIn: 1.2,
    /** Inset from the back cover's bottom-right trim corner. */
    insetIn: 0.25,
};
/** Minimum spine width before KDP allows printed spine text. */
exports.MIN_SPINE_TEXT_WIDTH_IN = 0.25;
/** Inches of spine width per interior page. */
exports.SPINE_RATES = {
    blackWhiteWhitePaper: 0.002252,
    blackWhiteCreamPaper: 0.0025,
    colourWhitePaper: 0.002347,
};
exports.DEFAULT_PAPER_TYPE = "blackWhiteWhitePaper";
function calculateSpineWidthIn(pageCount, paperType) {
    return pageCount * exports.SPINE_RATES[paperType];
}
/**
 * Full wraparound cover dimensions, computed dynamically — never hard-code
 * a finished canvas size.
 */
function calculateCoverDimensions(opts) {
    const trim = exports.TRIM_SIZES[opts.trimSizeId];
    if (!trim)
        throw new Error(`Unknown trim size: ${opts.trimSizeId}`);
    const bleedIn = opts.bleedIn ?? exports.COVER_BLEED_IN;
    const spineIn = calculateSpineWidthIn(opts.pageCount, opts.paperType);
    return {
        spineIn,
        totalWidthIn: bleedIn + trim.widthIn + spineIn + trim.widthIn + bleedIn,
        totalHeightIn: bleedIn + trim.heightIn + bleedIn,
        bleedIn,
    };
}
// ---------------------------------------------------------------------------
// Generation queue defaults (referenced by the Phase 3 queue implementation).
// ---------------------------------------------------------------------------
exports.MAX_CONCURRENT_GENERATIONS = 3;
exports.MAX_GENERATION_RETRIES = 2;
// ---------------------------------------------------------------------------
// KDP paperback page-count limits (black ink, 8.5×11).
// ---------------------------------------------------------------------------
exports.KDP_MIN_PAGE_COUNT = 24;
exports.KDP_MAX_PAGE_COUNT_BW = 828;
/** PDF points per inch (PDF user-space unit). */
exports.PDF_POINTS_PER_INCH = 72;
