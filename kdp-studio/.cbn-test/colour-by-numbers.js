...
exports.processColourByNumbers = processColourByNumbers;
const sharp_1 = __importDefault(require("sharp"));
const kdp_spec_1 = require("./kdp-spec");
...
const PRINT_W = kdp_spec_1.INTERIOR_IMAGE.widthPx; // 2550
const PRINT_H = kdp_spec_1.INTERIOR_IMAGE.heightPx; // 3300
...
function nearestColourName(r, g, b, used) {
...
async function processColourByNumbers(input) {
    ...
    const fitted = await (0, sharp_1.default)(input.image)
        ...
        .toBuffer();
    const { data, info } = await (0, sharp_1.default)(fitted)
        .raw()
    ...
    const segment = () => {
        ...
        for (let start = 0; start < gw * gh; start++) {
            ...
            while (qh < qt) {
                ...
                const x = p % gw;
                const y = (p / gw) | 0;
                if (x > 0 && labels[p - 1] === -1 && colourIdx[p - 1] === col) {
                    labels[p - 1] = id;
                    queue[qt++] = p - 1;
                }
                if (x < gw - 1 && labels[p + 1] === -1 && colourIdx[p + 1] === col) {
                    labels[p + 1] = id;
                    queue[qt++] = p + 1;
                }
                if (y > 0 && labels[p - gw] === -1 && colourIdx[p - gw] === col) {
                    labels[p - gw] = id;
                    queue[qt++] = p - gw;
                }
                if (y < gh - 1 && labels[p + gw] === -1 && colourIdx[p + gw] === col) {
                    labels[p + gw] = id;
                    queue[qt++] = p + gw;
                }
            }
    ...
    const distanceTransform = () => {
        ...
        while (qh < qt) {
            ...
            const x = p % gw;
            const y = (p / gw) | 0;
            if (x > 0 && dist[p - 1] === -1 && labels[p - 1] === l) {
                dist[p - 1] = d + 1;
                queue[qt++] = p - 1;
            }
            if (x < gw - 1 && dist[p + 1] === -1 && labels[p + 1] === l) {
                dist[p + 1] = d + 1;
                queue[qt++] = p + 1;
            }
            if (y > 0 && dist[p - gw] === -1 && labels[p - gw] === l) {
                dist[p - gw] = d + 1;
                queue[qt++] = p - gw;
            }
            if (y < gh - 1 && dist[p + gw] === -1 && labels[p + gw] === l) {
                dist[p + gw] = d + 1;
                queue[qt++] = p + gw;
            }
        }
    ...
    for (let pass = 0; pass < 6; pass++) {
        ...
        for (let p = 0; p < gw * gh; p++) {
            if (dist[p] > maxDist[labels[p]])
                maxDist[labels[p]] = dist[p];
        }
        ...
        const bump = (a, b) => {
            ...
            if (!m)
                borderCount.set(a, (m = new Map()));
            m.set(b, (m.get(b) ?? 0) + 1);
        };
        for (let p = 0; p < gw * gh; p++) {
            const x = p % gw;
            const l = labels[p];
            if (x < gw - 1 && labels[p + 1] !== l) {
                bump(l, labels[p + 1]);
                bump(labels[p + 1], l);
            }
            if (((p / gw) | 0) < gh - 1 && labels[p + gw] !== l) {
                bump(l, labels[p + gw]);
                bump(labels[p + gw], l);
            }
        }
        // Recolour doomed regions to their dominant neighbour's colour.
        const newColourOf = new Map();
        for (let r = 0; r < regionCount; r++) {
            const doomed = regionColour[r] !== -1 &&
            ...
            for (const [n, shared] of neighbours) {
                ...
                const nDoomed = regionColour[n] !== -1 &&
            ...
            if (bestN >= 0)
                newColourOf.set(r, regionColour[bestN]);
        }
        if (newColourOf.size === 0)
            break;
        for (let p = 0; p < gw * gh; p++) {
    ...
    const usedColours = new Map(); // colour key -> area
    for (let r = 0; r < regionCount; r++) {
        if (regionColour[r] === -1)
            continue;
    ...
    if (input.customPalette && input.customPalette.length > 0) {
        ...
        for (const [k, num] of remappedNumbers)
            colourToNumber.set(k, num);
        segment();
    }
    else {
        let n = 1;
    ...
    const sizeFor = (d) => Math.max(16, Math.min(46, Math.floor(d * 1.1)));
    const regions = [];
    const numberMarks = [];
    const validation = [];
    const regionNumber = new Map();
    const marksOfRegion = new Map();
    for (let r = 0; r < regionCount; r++) {
    ...
    for (const [r, sites] of sitesOfRegion) {
        ...
        for (const s of sites) {
            ...
            const x = s.p % gw;
            const y = (s.p / gw) | 0;
            if (placed.some((m) => (m.x - x) ** 2 + (m.y - y) ** 2 < SPACING * SPACING))
                continue;
            placed.push({ x, y });
    ...
    if (regions.length > params.targetMax * 1.8) {
        validation.push(`${regions.length} regions is well above the ${params.targetMax} ideal for this difficulty.`);
    }
    if (regions.length < 3) {
    ...
    const outlinePng = await (0, sharp_1.default)(outline, {
    ...
    for (const m of numberMarks) {
        ...
        svgParts.push(`<text x="${x}" y="${y}" font-size="${fs}" font-family="Arial, Helvetica, sans-serif" font-weight="bold" fill="#111111" stroke="#ffffff" stroke-width="${Math.max(2, fs * 0.14)}" paint-order="stroke" text-anchor="middle" dominant-baseline="central">${m.number}</text>`);
    }
    const keySvg = (withColours) => {
        if (input.keyPlacement !== "bottom")
            return "";
        ...
        palette.forEach((p, i) => {
            ...
            parts.push(`<circle cx="${cx}" cy="${cy}" r="36" fill="${withColours ? p.hex : "#ffffff"}" stroke="#111111" stroke-width="4"/>`, `<text x="${cx}" y="${cy}" font-size="40" font-family="Arial, Helvetica, sans-serif" font-weight="bold" fill="${numberFill}" text-anchor="middle" dominant-baseline="central">${p.number}</text>`, `<text x="${cx + 52}" y="${cy}" font-size="38" font-family="Arial, Helvetica, sans-serif" fill="#111111" dominant-baseline="central">${p.name}</text>`);
        });
        return parts.join("");
    };
    const overlaySvg = Buffer.from(`<svg xmlns="http://www.w3.org/2000/svg" width="${PRINT_W}" height="${PRINT_H}">${svgParts.join("")}${keySvg(false)}</svg>`);
    const numberedPage = await (0, sharp_1.default)({
    ...
    const colourPng = await (0, sharp_1.default)(colourRaw, {
        raw: { width: gw, height: gh, channels: 3 },
    })
        .resize(gw * scale, gh * scale, { kernel: "nearest" })
        .png()
        .toBuffer();
    const referenceSvg = Buffer.from(`<svg xmlns="http://www.w3.org/2000/svg" width="${PRINT_W}" height="${PRINT_H}">${keySvg(true)}</svg>`);
    const reference = await (0, sharp_1.default)({
        create: { width: PRINT_W, height: PRINT_H, channels: 3, background: "#ffffff" },
    })
        .composite([
